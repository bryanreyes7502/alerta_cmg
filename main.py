...
def descargar_csv(fecha: str) -> Path | None:
    ...
    log(f"Solicitando CSV al Coordinador para {fecha}...")
    ...
    if not response.content:
        log("La respuesta llegó vacía.")
        return None

    # El endpoint puede entregar CSV directamente o mediante headers de descarga.
    content_type = (response.headers.get("Content-Type") or "").lower()
    content_disposition = response.headers.get("Content-Disposition") or ""

    # Rechazo temprano de respuestas HTML inesperadas.
    sample = response.content[:300].lstrip().lower()
    if "text/html" in content_type and b"<html" in sample:
        raise RuntimeError("El endpoint devolvió HTML en vez del CSV esperado.")

    timestamp = ahora_local().strftime("%Y-%m-%d_%H-%M-%S")
    path = DOWNLOAD_DIR / f"energia_{fecha}_{timestamp}.csv"

    path.write_bytes(response.content)
    log(
        f"CSV descargado ({len(response.content):,} bytes). "
        f"Content-Type={content_type!r}; {content_disposition!r}"
    )
    return path
